...
TEST(SearchTest, CBID) {
	...
	string s2 = "Booth Darwin m 7 Albion Street british";

	EXPECT_TRUE(db.CBID_Search_by_CNIC("3740") == s1);
	EXPECT_TRUE(db.CBID_Search_by_CNIC("6027") == s2);
...
TEST(UpdateTest, CBID_Nationality) {
	...
	EXPECT_TRUE(db.updateCBIDNationality("european", "8372") == 1);
...
TEST(CrimeAddTest, CCID) {
	...
	string s1="Edda Oxford f 12 Baker Street british Robbery 2 years in prison 1000";

	EXPECT_TRUE(db.CCID_Search_by_CNIC("7564") == s1);
...
TEST(CrimeDeleteTest, CCID) {
	...
	string s1 = "Halsey Palmer f 11 Ann Street british";

	EXPECT_TRUE(db.CBID_Search_by_CNIC("5960") == s1);
...
TEST(CrimeUpdateTest, CCID) {
	...
	string s1 = "Halsey Palmer f 11 Ann Street british murder 20 years in prison 10000";
